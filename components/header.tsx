...
export function Header({
  ...
  const currentChapter = course?.chapters.find((chapter) => chapter.id === currentChapterId)

  return (
    <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className={currentChapterId ? "w-full p-2 px-8" : "container mx-auto p-2"}>
        ...
        <div className="p-4 space-y-2">
          {course?.chapters.map((chapter) => (
            <DrawerClose key={chapter.id} asChild>
              <Button
                variant={chapter.title === currentChapterTitle ? "default" : "ghost"}
